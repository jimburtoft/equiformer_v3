...
class EdgeDegreeEmbedding(torch.nn.Module):
    ...
    def forward_dense(
        ...
        """
        Dense padded forward pass for EdgeDegreeEmbedding.

        Replaces scatter-add with reshape [N*K] -> [N, K] + masked sum(dim=1).
        The SO3Rotation wigner_inv must be pre-set with shape [N*K, M, M].

        Args:
            atomic_numbers: [N] long tensor
            edge_distance: [N*K, D] radial basis expanded distances
            neighbor_idx: [N, K] neighbor indices
            num_nodes: int, N
            max_neighbors: int, K
            edge_envelope_weight: [N*K, 1] or None
            mask: [N, K] bool tensor
        Returns:
            outputs: [N, M_full, C] where M_full = (lmax+1)^2
        """
        N = num_nodes
        K = max_neighbors

        if self.use_atom_edge_embedding:
            # Source: neighbors, Target: center atoms (repeated K times)
            source_element = atomic_numbers[neighbor_idx.view(-1)]  # [N*K]
            target_element = (
        ...
        x_edge_m0 = self.rad_func(x_edge)  # [N*K, (lmax+1)*C]
        ...
        # Dense aggregation: reshape [N*K, M, C] -> [N, K, M, C], mask, sum
        x_edge = x_edge.view(N, K, x_edge.shape[1], x_edge.shape[2])
        if mask is not None:
            x_edge = x_edge * mask.unsqueeze(-1).unsqueeze(-1)
        outputs = x_edge.sum(dim=1)  # [N, M, C]
        outputs = outputs / self.rescale_factor
        return outputs
