...
class EquivariantMergeLayerNorm(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        # for L = 0
        if self.centering:
            scalars, rest = torch.split(inputs, [1, inputs.shape[1] - 1], dim=1)
            scalars_mean = scalars.mean(dim=2, keepdim=True)  # [N, 1, 1]
            scalars = scalars - scalars_mean
            inputs = torch.cat((scalars, rest), dim=1)
        ...
        if self.affine and self.centering:
            ...
            outputs = torch.cat(
                [first + self.affine_bias.view(1, 1, self.num_channels), rest], dim=1
            )
