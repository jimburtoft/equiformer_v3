...
class SO3Embedding:
    ...
    def _m_primary(self, mapping):
        self.embedding = torch.einsum(
            "nac, ba -> nbc", self.embedding, mapping.to_m.to(self.embedding.dtype)
        )

    # Reshape the embedding m -> l
    def _l_primary(self, mapping):
        self.embedding = torch.einsum(
            "nac, ab -> nbc", self.embedding, mapping.to_m.to(self.embedding.dtype)
        )
...
class SO3Grid(torch.nn.Module):
    ...
    def to_grid(self, embedding):
        ...
        grid = torch.einsum(
            "aj, njc -> nac", self.to_grid_mat.to(embedding.dtype), embedding
        )
    ...
    def from_grid(self, grid):
        ...
        embedding = torch.einsum(
            "ja, nac -> njc", self.from_grid_mat.to(grid.dtype), grid
        )
