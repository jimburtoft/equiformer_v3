...
from .layer_norm import get_normalization_layer, RMSNorm
from .radial_function import RadialFunction
from .so2_ops import SO2Linear
from .so3 import SO3Linear, SO3Grid
from .utils import reduce_edge
from .drop import GraphDropPath, EquivariantDropout
    ...
    def forward(self, inputs):
        ...
        if self.use_grid_mlp:
            ...
            if "gates2" not in self.activation:
                ...
                if "_mem" not in self.activation:
                    output_grid = self.grid_mlp(output_grid, input_scalar)
                else:
                    output_grid = torch.utils.checkpoint.checkpoint(
                        self.grid_mlp, output_grid, input_scalar
                    )
