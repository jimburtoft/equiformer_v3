...
class EquiformerV3_OC(torch.nn.Module, GraphModelMixin):
    ...
    def forward_static(
        ...
        """
        Forward pass that bypasses generate_graph() for torch.compile compatibility.
        Accepts pre-computed graph tensors directly (static shapes for compilation).

        Args:
            atomic_numbers: [N_atoms] long tensor
            edge_index: [2, E] long tensor
            edge_distance: [E] float tensor (scalar distances)
            edge_distance_vec: [E, 3] float tensor (distance vectors)
            batch: [N_atoms] long tensor (graph membership)
        ...
        self.dtype = edge_distance_vec.dtype
        self.device = edge_distance_vec.device

        source_atomic_numbers = atomic_numbers[edge_index[0]]
        target_atomic_numbers = atomic_numbers[edge_index[1]]

        edge_distance_expanded, edge_envelope_weight = self._forward_edge(
            edge_distance, edge_distance_vec
        )
        x = self._forward_embedding(
