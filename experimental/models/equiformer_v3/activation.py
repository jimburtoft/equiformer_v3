...
class SeparableGateS2Activation_SwiGLU_Merge(GateActivation):
    ...
    def forward(self, inputs, scalars):
        ...
        output_scalars, gate_scalars = torch.split(
            scalars, [inputs.shape[2], scalars.shape[2] - inputs.shape[2]], dim=2
        )
        ...
        # 6. Merge (use cat instead of in-place assignment to avoid slice_scatter in backward)
        first, rest = torch.split(
            output_vectors, [1, output_vectors.shape[1] - 1], dim=1
        )
        outputs = torch.cat([first + output_scalars, rest], dim=1)
        return outputs
