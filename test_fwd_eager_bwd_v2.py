"""Neuron compiled forward + eager backward - fix view->reshape in bwd graph."""
...
# Revert custom scatter_ops
import equiformer_v3.scatter_ops as scatter_ops_mod
def scatter_add_plain(src, index, output_size):
    output = torch.zeros(output_size, *src.shape[1:], device=src.device, dtype=src.dtype)
...
print('=' * 60)
print('Neuron compiled forward + eager backward (view->reshape fix)')
print('=' * 60)
...
def fix_views_in_graph(gm):
    """Replace aten.view with aten.reshape in graph to handle non-contiguous tensors."""
...
def eager_bw_compiler(gm, example_inputs):
    """Eager backward with view->reshape fix."""
    gm = fix_views_in_graph(gm)
    print(f'  [BW-EAGER] Backward graph ({len(list(gm.graph.nodes))} nodes) - eager with reshape fix')
    return make_boxed_func(gm.forward)

def neuron_fwd_eager_bwd_backend(gm, example_inputs):
    ...
    aot_backend = aot_autograd(
        ...
        bw_compiler=eager_bw_compiler,
        keep_inference_input_mutations=True,
        decompositions=get_compile_decomposition_table(),
    )
    return aot_backend(gm, example_inputs)

register_backend(name='neuron_hybrid', compiler_fn=neuron_fwd_eager_bwd_backend)
...
compiled_fn = torch.compile(model.forward_static, backend='neuron_hybrid')
...
for step in range(5):
    ...
    print(f'  Step {step+1}: loss={loss.cpu().item():.6f} time={elapsed:.1f}s')

has_grad = sum(1 for p in model.parameters() if p.grad is not None)
total_params = sum(1 for _ in model.parameters())
print(f'\n  Gradients: {has_grad}/{total_params}')
print(f'  Status: {"PASS" if has_grad == total_params else "FAIL"}')
print('\nDONE')
