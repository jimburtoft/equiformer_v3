"""
EquiformerV3 inference on AWS Trainium (Neuron).
...
def main():
    ...
    print(f"Config: {NUM_ATOMS} atoms, {MAX_NEIGHBORS} neighbors")
    print()
    ...
    atomic_numbers = torch.randint(1, 95, (NUM_ATOMS,))
    positions = torch.randn(NUM_ATOMS, 3) * 5.0
    inputs = prepare_inputs(atomic_numbers, positions, MAX_NEIGHBORS, DEVICE)
    total_edges = inputs["mask"].sum().item()
    print(f"  Total edges: {total_edges}")

    # --- Compile ---
    print("\nCompiling model (first run: 5-20 min, cached afterward)...")
    ...
    print(f"  Output:      energy shape = {output['energy'].shape}")
    print(f"{'=' * 60}")

    # --- CPU Comparison ---
    print("\nRunning CPU comparison...")
    model_cpu = create_model()
    inputs_cpu = prepare_inputs(atomic_numbers, positions, MAX_NEIGHBORS, "cpu")
    with torch.no_grad():
        for _ in range(3):
            model_cpu.forward_dense(**inputs_cpu)
        cpu_times = []
        for _ in range(10):
            t0 = time.perf_counter_ns()
            model_cpu.forward_dense(**inputs_cpu)
